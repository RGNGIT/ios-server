...
class Tests {
  ...
  async getTest(req: Request, res: Response): Promise<void> {
    try {
      ...
      const iosDifficulty = await TestService.fetchTestTopicMaterialDifficulty(req.params.id);
      ...
      for await (let i of res1) {
        ...
        for await (let answer of res2) {
          ans.push({
            ...
            CorrectFloat: GetCorrect == "true" ? answer.IsCorrect : undefined,
          });
        }
        /*
